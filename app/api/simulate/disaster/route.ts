  ...
  try {
    ...
    simulationResult.affectedZones.forEach((zone, index) => {
      ...
      reports.push({
        ...
        description: `${zone.name} experiencing ${urgency.toLowerCase()} impact from ${disasterType}. Impact level: ${zone.impactLevel}/10`,
        urgency: urgency,
        latitude: zone.lat,
        longitude: zone.lon,
    ...
    simulationResult.resourceAllocation.forEach((resource, index) => {
      if (resource.count > 0) {
        ...
        reports.push({
          ...
          description: `Deploying ${resource.count} units of ${resource.type} for disaster response`,
